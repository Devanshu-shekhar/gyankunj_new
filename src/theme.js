...
export const useMode = () => {
  ...
  useEffect(() => {
    ...
    if (configStr) {
      try {
        ...
        const faviconData = parsedConfig.favicon_info;
        const base64Url = `data:image/${faviconData.mime_type};base64,${faviconData.data}`;
  ...
  const updateFavicon = (href) => {
    ...
    link.rel = 'icon';
    link.type = 'image/png';
    link.href = href || Gyankoonj_fav; // fallback if missing
